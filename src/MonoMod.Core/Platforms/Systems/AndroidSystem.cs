...
namespace MonoMod.Core.Platforms.Systems
{
    internal sealed class AndroidSystem : ISystem, IInitialize<IArchitecture>
    {
        ...
        {
            // idk why but removing this line causes errno to be always 2
            _ = AndroidNative.Errno;
            
            PageSize = (nint)AndroidNative.Sysconf(AndroidNative.SysconfName._SC_PAGESIZE);
        ...
        private void ProtectRW(IntPtr addr, nint size)
        {
            ...
            if (AndroidNative.Mprotect(addr, (nuint)size, AndroidNative.Protection.Read | AndroidNative.Protection.Write) != 0)
            {
                throw new Win32Exception(AndroidNative.Errno);
            }
        }
        ...
        {
            RoundToPageBoundary(ref addr, ref size);
            if (AndroidNative.Mprotect(addr, (nuint)size, AndroidNative.Protection.Read | AndroidNative.Protection.Write | AndroidNative.Protection.Execute) != 0)
            {
                throw new Win32Exception(AndroidNative.Errno);
            }
        }
        ...
        private sealed class MmapPagedMemoryAllocator : PagedMemoryAllocator
        {
            ...
            static unsafe MmapPagedMemoryAllocator()
            {
                ...
                if (AndroidNative.Pipe2(pipefd, AndroidNative.PipeFlags.CloseOnExec) == -1)
                {
                    throw new Win32Exception(AndroidNative.Errno, "Failed to create pipe for page probes");
                }
            ...
            public static unsafe bool PageAllocated(nint page)
            {
                ...
                if (AndroidNative.Mincore(page, 1, &garbage) == -1)
                {
                    var lastError = AndroidNative.Errno;
                    if (lastError == 12)
            ...
            public static unsafe bool PageReadable(nint page)
            {
                ...
                if (AndroidNative.Write(PageProbePipeWriteFD, page, 1) == -1)
                {
                    var lastError = AndroidNative.Errno;
                    if (lastError == 14)
                ...
                byte garbage;
                if (AndroidNative.Read(PageProbePipeReadFD, new IntPtr(&garbage), 1) == -1)
                {
            ...
            {
                var prot = request.Executable ? AndroidNative.Protection.Execute : AndroidNative.Protection.None;
                prot |= AndroidNative.Protection.Read | AndroidNative.Protection.Write;

                // mmap the page we found
                var mmapPtr = AndroidNative.Mmap(IntPtr.Zero, (nuint)PageSize, prot, AndroidNative.MmapFlags.Private | AndroidNative.MmapFlags.Anonymous, -1, 0);
                if (mmapPtr is 0 or -1)
                {
                    // fuck
                    var errno = AndroidNative.Errno;
            ...
            )
            {
                ...
                var prot = request.Base.Executable ? AndroidNative.Protection.Execute : AndroidNative.Protection.None;
                prot |= AndroidNative.Protection.Read | AndroidNative.Protection.Write;
                ...
                // mmap the page we found
                var mmapPtr = AndroidNative.Mmap(ptr, (nuint)PageSize, prot, AndroidNative.MmapFlags.Private | AndroidNative.MmapFlags.Anonymous | AndroidNative.MmapFlags.FixedNoReplace, -1, 0);
                if (mmapPtr is 0 or -1)
                {
                    // fuck
                    allocated = null;
            ...
            {
                var res = AndroidNative.Munmap(page.BaseAddr, page.Size);
                if (res != 0)
                {
                    errorMsg = new Win32Exception(AndroidNative.Errno).Message;
                    return false;
                }
                errorMsg = null;
                return true;
        ...
        private unsafe PosixExceptionHelper CreateNativeExceptionHelper()
        {
            ...
            try
            {
                ...
                fixed (byte* pTmpl = templ)
                    fd = AndroidNative.MkSTemp(pTmpl);
